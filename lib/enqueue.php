...
function theme_scripts() {
  ...
  $themeJs = get_stylesheet_directory_uri() . '/assets/dist/js/theme.js';
  ...
  if (!wp_is_mobile && preg_match('~MSIE|Internet Explorer~i', $_SERVER['HTTP_USER_AGENT']) || (strpos($_SERVER['HTTP_USER_AGENT'], 'Trident/7.0; rv:11.0') !== false)) {
    wp_enqueue_script('polyfill', 'https://cdnjs.cloudflare.com/ajax/libs/babel-polyfill/7.2.5/polyfill.min.js', array('jquery'), null);
    $themeJs = get_stylesheet_directory_uri() . '/assets/dist/js/theme-ie.js';
  }

  wp_enqueue_script('theme-js', $themeJs, array('jquery'), filemtime( get_theme_file_path('assets/dist/js/theme.js') ), true);
